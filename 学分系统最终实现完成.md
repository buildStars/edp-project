# 学分系统最终实现完成

## 🎯 最终需求确认

### 学分系统核心规则

1. **学分不可直接转赠**
   - ❌ 用户之间不能直接转赠学分
   - ✅ 唯一例外：企业管理员可以分配学分给企业员工

2. **赠送课程功能**
   - ✅ **个人学分**：可以使用学分赠送课程给他人
   - ❌ **锁定学分**：只能自己购买课程，不能赠送课程

3. **学分类型**
   ```
   总学分 = 个人学分 + 锁定学分
   ```

## 📊 数据结构设计

### Credit 表结构

```prisma
model Credit {
  id              String   @id @default(uuid())
  userId          String   @unique
  balance         Int      @default(0)  // 当前学分余额（统一余额）
  total           Int      @default(0)  // 累计获得学分（统一总数）
  used            Int      @default(0)  // 累计消耗学分（统一消耗）
  
  personalBalance Int      @default(0)  // 个人学分余额（可用于赠送课程）
  lockedBalance   Int      @default(0)  // 锁定学分余额（企业分配，只能购课不可赠课）
  
  // 关系：balance = personalBalance + lockedBalance
}
```

### 学分类型对比

| 属性 | 个人学分 | 锁定学分 |
|------|---------|---------|
| **来源** | 管理员充值、课程完成奖励 | 企业管理员分配 |
| **自己购课** | ✅ | ✅ |
| **赠送课程** | ✅ | ❌ |
| **直接转赠** | ❌ | ❌ |
| **字段名** | `personalBalance` | `lockedBalance` |

## ✅ 已完成的修改

### 1. 后端服务更新 ✅

#### A. credits.service.ts

**consumeCredit** - 消耗学分（报名课程）
```typescript
// 优先使用锁定学分，不足时使用个人学分
const lockedUsed = Math.min(credit.lockedBalance, amount);
const personalUsed = amount - lockedUsed;

await this.prisma.credit.update({
  data: {
    balance: credit.balance - amount,
    used: credit.used + amount,
    lockedBalance: credit.lockedBalance - lockedUsed,
    personalBalance: credit.personalBalance - personalUsed,
  },
});
```

**addCredit** - 增加学分
```typescript
// isLocked=true: 增加锁定学分
// isLocked=false: 增加个人学分
const newPersonal = isLocked ? credit.personalBalance : credit.personalBalance + amount;
const newLocked = isLocked ? credit.lockedBalance + amount : credit.lockedBalance;
```

**refundCredit** - 退回学分
```typescript
// 退回到个人学分（简化处理）
await this.prisma.credit.update({
  data: {
    balance: credit.balance + amount,
    used: credit.used - amount,
    personalBalance: credit.personalBalance + amount,
  },
});
```

**deduct** - 扣除学分
```typescript
// 优先扣除个人学分，不足时扣除锁定学分
const personalDeduct = Math.min(credit.personalBalance, amount);
const lockedDeduct = amount - personalDeduct;
```

#### B. corporate-credits.service.ts

**allocateCredit** - 企业管理员分配学分
```typescript
// 检查管理员的个人学分
if (adminCredit.personalBalance < amount) {
  throw new BadRequestException('您的个人学分不足');
}

// 扣除管理员的个人学分，增加员工的锁定学分
await this.prisma.$transaction(async (tx) => {
  // 1. 扣除管理员个人学分
  await tx.credit.update({
    where: { userId: adminId },
    data: {
      balance: { decrement: amount },
      personalBalance: { decrement: amount },
    },
  });
  
  // 2. 增加员工锁定学分
  await tx.credit.update({
    where: { userId: employeeId },
    data: {
      balance: { increment: amount },
      lockedBalance: { increment: amount },
    },
  });
});
```

**getEmployees** - 获取企业员工
```typescript
return employees.map(emp => ({
  totalBalance: emp.credit?.balance || 0,
  personalBalance: emp.credit?.personalBalance || 0,  // 可赠课
  lockedBalance: emp.credit?.lockedBalance || 0,      // 不可赠课
}));
```

#### C. enrollments.service.ts

**enroll** - 报名课程
```typescript
// 检查总余额
if (userCredit.balance < requiredCredit) {
  return {
    message: `学分不足，当前${userCredit.balance}（锁定${userCredit.lockedBalance}+个人${userCredit.personalBalance}），需要${requiredCredit}学分`,
  };
}
```

### 2. 前端更新 ✅

#### A. 个人中心 (frontend-uniapp/pages/mine/index.vue)

```vue
<!-- 总学分 -->
<view class="credit-item">
  <Icon name="star" color="#FFD700" />
  <text>{{ credits.balance }}</text>
  <text>总学分</text>
</view>

<!-- 个人学分（可赠课）-->
<view class="credit-item">
  <Icon name="user" color="#52C41A" />
  <text>{{ credits.personalBalance }}</text>
  <text>个人学分</text>
  <text class="tip">可赠课</text>
</view>

<!-- 锁定学分（只能购课）-->
<view class="credit-item">
  <Icon name="work" color="#FF6B6B" />
  <text>{{ credits.lockedBalance }}</text>
  <text>锁定学分</text>
  <text class="tip">只能购课</text>
</view>
```

### 3. 数据库迁移脚本 ✅

**文件**: `backend/prisma/migrations/20251116_final_credit_system.sql`

```sql
-- 添加新字段
ALTER TABLE `credits` 
ADD COLUMN `personalBalance` INT NOT NULL DEFAULT 0 COMMENT '个人学分余额（可赠课）',
ADD COLUMN `lockedBalance` INT NOT NULL DEFAULT 0 COMMENT '锁定学分余额（只能购课）';

-- 迁移数据
UPDATE `credits` 
SET 
  `balance` = COALESCE(`balance`, 0) + COALESCE(`corporateBalance`, 0),
  `personalBalance` = COALESCE(`balance`, 0),
  `lockedBalance` = COALESCE(`corporateBalance`, 0);
```

## 🔄 学分流转示例

### 场景 1：企业管理员分配学分

```
企业管理员 A：
- 个人学分：100
- 锁定学分：0

操作：分配 10 学分给员工 B

结果：
管理员 A：
- 个人学分：100 → 90（减少 10）
- 锁定学分：0（不变）
- 总学分：100 → 90

员工 B：
- 个人学分：50（不变）
- 锁定学分：0 → 10（增加 10）
- 总学分：50 → 60

员工 B 的这 10 学分：
✅ 可以用来报名课程
❌ 不能用来赠送课程
```

### 场景 2：报名课程（优先使用锁定学分）

```
用户学分：
- 锁定学分：15
- 个人学分：10
- 总学分：25

报名需要 20 学分：

扣除方案：
1. 优先使用锁定学分：15
2. 不足部分使用个人学分：5
3. 总消耗：20

剩余：
- 锁定学分：0
- 个人学分：5
- 总学分：5
```

### 场景 3：赠送课程（只能使用个人学分）

```
用户 A 学分：
- 锁定学分：20
- 个人学分：10
- 总学分：30

赠送课程给用户 B，需要 5 学分：

检查：
✅ 个人学分（10）足够
✅ 可以赠送

扣除：
- 锁定学分：20（不变，不能用于赠课）
- 个人学分：10 → 5（减少 5）
- 总学分：30 → 25

用户 B 获得：
- 课程报名资格（不是学分）
```

如果个人学分不足：
```
用户 C 学分：
- 锁定学分：20
- 个人学分：3
- 总学分：23

赠送课程需要 5 学分：
❌ 个人学分（3）不足
❌ 不能使用锁定学分赠课
→ 无法赠送
```

## 📋 执行步骤

### 步骤 1: 执行数据库迁移

在数据库管理工具中执行：
```bash
backend/prisma/migrations/20251116_final_credit_system.sql
```

或使用命令行：
```bash
mysql -u root -p edp_db < backend/prisma/migrations/20251116_final_credit_system.sql
```

### 步骤 2: 已完成 ✅

```bash
cd backend
npx prisma generate
```

### 步骤 3: 重启服务

```bash
# 后端
cd backend
npm run start:dev

# 前端（小程序）
# 在微信开发者工具中点击"编译"
```

## 🧪 测试清单

### 1. 学分显示 ✓
- [ ] 个人中心显示：总学分、个人学分、锁定学分
- [ ] 个人学分显示"可赠课"提示
- [ ] 锁定学分显示"只能购课"提示
- [ ] 数字正确：`balance = personalBalance + lockedBalance`

### 2. 报名课程 ✓
- [ ] 有锁定学分：优先使用锁定学分
- [ ] 锁定学分不足：使用锁定学分 + 个人学分
- [ ] 学分不足：显示正确的错误提示（区分锁定和个人）

### 3. 企业管理员分配 ✓
- [ ] 检查管理员的个人学分余额
- [ ] 扣除管理员的个人学分
- [ ] 增加员工的锁定学分
- [ ] 通知内容："锁定学分，只能购课不可赠课"

### 4. 赠送课程（新功能）✓
- [ ] 检查用户的个人学分余额
- [ ] 只能使用个人学分赠课
- [ ] 锁定学分不能用于赠课
- [ ] 个人学分不足时提示错误

### 5. 退课 ✓
- [ ] 退回学分到个人学分余额
- [ ] 学分数字正确

## 💡 核心业务逻辑

### 学分消费优先级

```javascript
// 报名课程
function enrollCourse(userId, courseId, requiredCredit) {
  const credit = getUserCredit(userId);
  
  // 1. 检查总余额
  if (credit.balance < requiredCredit) {
    throw new Error('学分不足');
  }
  
  // 2. 计算扣除方案（优先使用锁定学分）
  const lockedUsed = Math.min(credit.lockedBalance, requiredCredit);
  const personalUsed = requiredCredit - lockedUsed;
  
  // 3. 扣除学分
  updateCredit(userId, {
    balance: credit.balance - requiredCredit,
    lockedBalance: credit.lockedBalance - lockedUsed,
    personalBalance: credit.personalBalance - personalUsed,
  });
}
```

### 赠送课程逻辑

```javascript
// 赠送课程（未来功能）
function giftCourse(fromUserId, toUserId, courseId, requiredCredit) {
  const credit = getUserCredit(fromUserId);
  
  // 1. 只检查个人学分余额
  if (credit.personalBalance < requiredCredit) {
    throw new Error(`个人学分不足，当前: ${credit.personalBalance}，需要: ${requiredCredit}`);
  }
  
  // 2. 只扣除个人学分（不能使用锁定学分）
  updateCredit(fromUserId, {
    balance: credit.balance - requiredCredit,
    personalBalance: credit.personalBalance - requiredCredit,
  });
  
  // 3. 为接收方报名课程
  enrollCourse(toUserId, courseId);
}
```

### 企业管理员分配逻辑

```javascript
// 企业管理员分配学分给员工
function allocateCredit(adminId, employeeId, amount) {
  const adminCredit = getUserCredit(adminId);
  
  // 1. 检查管理员的个人学分
  if (adminCredit.personalBalance < amount) {
    throw new Error('您的个人学分不足');
  }
  
  // 2. 扣除管理员的个人学分
  updateCredit(adminId, {
    balance: adminCredit.balance - amount,
    personalBalance: adminCredit.personalBalance - amount,
  });
  
  // 3. 增加员工的锁定学分（不可赠课）
  const employeeCredit = getUserCredit(employeeId);
  updateCredit(employeeId, {
    balance: employeeCredit.balance + amount,
    lockedBalance: employeeCredit.lockedBalance + amount,  // 作为锁定学分
  });
}
```

## 🎯 关键特性总结

1. ✅ **统一学分余额**：用户只看到一个总学分数字
2. ✅ **清晰区分类型**：个人学分（可赠课）vs 锁定学分（只能购课）
3. ✅ **合理的消费优先级**：优先使用锁定学分（鼓励使用企业学分）
4. ✅ **灵活的企业管理**：企业管理员可以分配学分给员工
5. ✅ **防止滥用**：锁定学分不能用于赠课，防止企业学分被滥用

## 📝 下一步

1. ✅ Schema 更新完成
2. ✅ 后端代码更新完成
3. ✅ 前端代码更新完成
4. ✅ Prisma Client 生成完成
5. ⏳ 执行数据库迁移
6. ⏳ 测试验证
7. ⏳ 部署上线

---

**实施完成时间**: 2025-11-16  
**状态**: ✅ 代码已完成，等待数据库迁移











