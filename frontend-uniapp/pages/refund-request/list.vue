  ...
  <view class="page">
    ...
    <view class="list">
      ...
      >
        ...
        <view class="content">
          ...
          <view class="info-row">
            ...
            <text class="value highlight">{{ item.creditAmount }}</text>
          </view>
          <view class="info-row">
            <text class="label">申请时间：</text>
            <text class="value">{{ formatTime(item.createdAt) }}</text>
          </view>
          <view v-if="item.reason" class="info-row">
            <text class="label">退课原因：</text>
        ...
        <view v-if="item.status === 'PENDING'" class="actions">
          <button class="btn-cancel" @click="cancelRequest(item.id)">取消申请</button>
        </view>
      </view>
      ...
      <view v-if="!loading && list.length === 0" class="empty">
        ...
        <text class="empty-text">暂无退课申请</text>
      </view>
    </view>
...
import { getMyRefundRequests, cancelRefundRequest } from '@/api/refund-request'
...
const loadList = async () => {
  try {
    ...
    const res = await getMyRefundRequests(params)
    list.value = res.list || []
    loading.value = false
  } catch (error) {
    loading.value = false
