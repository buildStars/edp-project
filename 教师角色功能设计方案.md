# 教师角色功能设计方案

## 📋 需求分析

### 核心痛点
1. 教师需要管理自己的学员
2. 教师需要查看课程相关数据
3. 教师需要管理课程签到和评价
4. 教师需要上传和管理课件

### 设计目标
- 数据隔离：教师只能看到自己的数据
- 功能完整：提供课程管理的全流程功能
- 界面友好：专门为教师设计的工作台

## 🎯 功能模块设计

### 1. 我的课程（教师工作台）

#### 1.1 课程概览
```
┌─────────────────────────────────────────────┐
│  我的课程 (共 5 门)                          │
├─────────────────────────────────────────────┤
│  统计卡片：                                  │
│  ┌───────┬───────┬───────┬───────┐          │
│  │ 进行中 │ 已完成 │ 总学员 │ 待签到 │          │
│  │   3   │   2   │  156  │   45  │          │
│  └───────┴───────┴───────┴───────┘          │
└─────────────────────────────────────────────┘
```

#### 1.2 课程列表（带快捷操作）
- 课程基本信息
- 报名人数/签到人数
- 快捷操作：
  - 开始签到
  - 查看学员
  - 课程评价
  - 上传课件
  - 编辑课程

### 2. 我的学员

#### 2.1 学员列表（只显示报名了自己课程的学员）
- 学员基本信息
- 报名课程列表
- 签到记录
- 学习进度
- 评价反馈

#### 2.2 学员详情
- 个人信息
- 报名课程
- 签到统计
- 评价历史
- 学分获得情况

### 3. 签到管理

#### 3.1 签到统计
- 实时签到率
- 签到趋势图
- 缺勤学员列表

#### 3.2 签到操作
- 开启签到（设置有效时间）
- 查看签到二维码
- 手动签到
- 导出签到表

### 4. 课程评价

#### 4.1 评价统计
- 评价率
- 平均评分
- 评价趋势
- 词云分析

#### 4.2 评价详情
- 学员评价列表
- 评价内容
- 评分分布
- 问题反馈

### 5. 课件管理

#### 5.1 课件库
- 按课程分类
- 文件列表
- 上传/删除
- 预览/下载

### 6. 教学数据

#### 6.1 数据看板
- 学员出勤率
- 课程完成率
- 评价满意度
- 学员增长趋势

## 🗄️ 数据库设计

### 新增表：教师-学员关系表

```prisma
// 教师-学员关系表（已存在 TeacherStudent）
model TeacherStudent {
  id         String   @id @default(uuid())
  teacherId  String   // 教师ID
  studentId  String   // 学员ID
  assignedBy String?  // 分配人ID
  assignedAt DateTime @default(now())
  notes      String?  // 备注
  
  teacher    User     @relation("TeacherRelations", fields: [teacherId], references: [id])
  student    User     @relation("StudentRelations", fields: [studentId], references: [id])
  
  @@unique([teacherId, studentId])
  @@index([teacherId])
  @@index([studentId])
  @@map("teacher_students")
}
```

### 新增：课程-教师关联

```prisma
// Course 表添加字段
model Course {
  // ... 现有字段
  teacherId     String?  // 主讲教师ID
  teacherName   String?  // 教师姓名（冗余字段，便于显示）
  // ... 其他字段
}
```

## 🔐 权限设计

### 数据权限规则

| 数据类型 | 教师权限 | 实现方式 |
|---------|---------|---------|
| 课程列表 | 只能看到 teacherId = 自己的课程 | SQL WHERE 过滤 |
| 学员列表 | 只能看到报名了自己课程的学员 | 关联查询 Enrollment |
| 签到记录 | 只能看到自己课程的签到 | 关联查询 Course |
| 评价记录 | 只能看到自己课程的评价 | 关联查询 Course |
| 课件管理 | 只能管理自己课程的课件 | 关联查询 Course |

### 功能权限

```typescript
// 教师角色新增权限
TEACHER: [
  'dashboard:view',           // 查看首页
  'my-courses:view',          // 查看我的课程（新增）
  'my-courses:manage',        // 管理我的课程（新增）
  'my-students:view',         // 查看我的学员（新增）
  'courses:view',             // 查看课程列表
  'courses:create',           // 创建课程
  'courses:edit',             // 编辑课程
  'chapters:view',            // 查看章节
  'chapters:manage',          // 管理章节
  'enrollments:view',         // 查看报名
  'enrollments:checkin',      // 签到管理
  'enrollments:evaluation',   // 评价管理
  'courseware:view',          // 查看课件
  'courseware:upload',        // 上传课件
  'statistics:view',          // 查看统计
]
```

## 💻 前端实现

### 路由配置

```typescript
// admin-frontend/src/router/index.ts
{
  path: '/teacher',
  name: 'Teacher',
  meta: {
    title: '教学管理',
    icon: 'UserFilled',
  },
  children: [
    {
      path: '/teacher/courses',
      name: 'TeacherCourses',
      component: () => import('@/views/Teacher/MyCourses.vue'),
      meta: {
        title: '我的课程',
        permission: 'my-courses:view',
      },
    },
    {
      path: '/teacher/students',
      name: 'TeacherStudents',
      component: () => import('@/views/Teacher/MyStudents.vue'),
      meta: {
        title: '我的学员',
        permission: 'my-students:view',
      },
    },
    {
      path: '/teacher/checkin/:courseId',
      name: 'TeacherCheckin',
      component: () => import('@/views/Teacher/CourseCheckin.vue'),
      meta: {
        title: '课程签到',
        hidden: true,
        activeMenu: '/teacher/courses',
      },
    },
    {
      path: '/teacher/evaluation/:courseId',
      name: 'TeacherEvaluation',
      component: () => import('@/views/Teacher/CourseEvaluation.vue'),
      meta: {
        title: '课程评价',
        hidden: true,
        activeMenu: '/teacher/courses',
      },
    },
  ],
}
```

### 页面结构

```
admin-frontend/src/views/Teacher/
├── MyCourses.vue          # 我的课程列表（工作台）
├── MyStudents.vue         # 我的学员列表
├── CourseCheckin.vue      # 课程签到管理
├── CourseEvaluation.vue   # 课程评价查看
└── components/
    ├── CourseCard.vue     # 课程卡片
    ├── StudentCard.vue    # 学员卡片
    └── CheckinStats.vue   # 签到统计
```

## 🔧 后端实现

### API 设计

#### 1. 教师课程相关

```typescript
// GET /api/teachers/my-courses
// 获取教师的课程列表
{
  courses: Course[],
  statistics: {
    totalCourses: number,
    activeCourses: number,
    totalStudents: number,
    pendingCheckins: number
  }
}

// GET /api/teachers/courses/:id/students
// 获取某课程的学员列表
{
  students: User[],
  statistics: {
    total: number,
    checkedIn: number,
    evaluated: number
  }
}
```

#### 2. 教师学员相关

```typescript
// GET /api/teachers/my-students
// 获取教师的所有学员（去重）
{
  students: User[],
  pagination: {
    total: number,
    page: number,
    pageSize: number
  }
}

// GET /api/teachers/students/:id
// 获取学员详情
{
  student: User,
  enrolledCourses: Course[],
  checkinRecords: Checkin[],
  evaluations: Evaluation[]
}

// POST /api/teachers/assign-students
// 管理员/教务给教师分配学员
{
  teacherId: string,
  studentIds: string[]
}
```

#### 3. 签到管理

```typescript
// GET /api/teachers/courses/:id/checkins
// 获取课程签到统计
{
  sessions: CheckinSession[],
  statistics: {
    totalSessions: number,
    averageAttendance: number,
    totalCheckins: number
  }
}

// POST /api/teachers/courses/:id/checkin/start
// 开始签到
{
  duration: number  // 签到有效时长（分钟）
}
```

#### 4. 评价管理

```typescript
// GET /api/teachers/courses/:id/evaluations
// 获取课程评价
{
  evaluations: Evaluation[],
  statistics: {
    totalCount: number,
    averageScore: number,
    scoreDistribution: {
      5: number,
      4: number,
      3: number,
      2: number,
      1: number
    }
  }
}
```

### 后端服务实现

```typescript
// backend/src/modules/teachers/teachers.service.ts

@Injectable()
export class TeachersService {
  constructor(private prisma: PrismaService) {}

  // 获取教师的课程列表（只返回自己的课程）
  async getMyCourses(teacherId: string) {
    const courses = await this.prisma.course.findMany({
      where: {
        teacherId,
        status: { not: 'DELETED' }
      },
      include: {
        _count: {
          select: {
            enrollments: {
              where: { status: 'ENROLLED' }
            }
          }
        }
      },
      orderBy: { startTime: 'desc' }
    });

    // 统计数据
    const statistics = {
      totalCourses: courses.length,
      activeCourses: courses.filter(c => c.status === 'PUBLISHED').length,
      totalStudents: courses.reduce((sum, c) => sum + c._count.enrollments, 0),
      pendingCheckins: 0 // 需要单独查询
    };

    return { courses, statistics };
  }

  // 获取教师的学员列表（只返回报名了自己课程的学员）
  async getMyStudents(teacherId: string, query: any) {
    // 先获取教师的所有课程
    const teacherCourses = await this.prisma.course.findMany({
      where: { teacherId },
      select: { id: true }
    });

    const courseIds = teacherCourses.map(c => c.id);

    // 查询报名了这些课程的学员（去重）
    const enrollments = await this.prisma.enrollment.findMany({
      where: {
        courseId: { in: courseIds },
        status: 'ENROLLED'
      },
      include: {
        user: true,
        course: {
          select: {
            id: true,
            title: true
          }
        }
      },
      distinct: ['userId']
    });

    return enrollments.map(e => ({
      ...e.user,
      enrolledCourse: e.course
    }));
  }

  // 获取课程的学员列表
  async getCourseStudents(teacherId: string, courseId: string) {
    // 验证课程是否属于该教师
    const course = await this.prisma.course.findFirst({
      where: {
        id: courseId,
        teacherId
      }
    });

    if (!course) {
      throw new ForbiddenException('无权访问该课程');
    }

    const enrollments = await this.prisma.enrollment.findMany({
      where: {
        courseId,
        status: 'ENROLLED'
      },
      include: {
        user: true,
        checkins: true,
        evaluations: true
      }
    });

    return enrollments.map(e => ({
      ...e.user,
      checkinCount: e.checkins.length,
      hasEvaluated: e.evaluations.length > 0
    }));
  }
}
```

## 📱 页面设计

### 1. 我的课程（工作台）

```vue
<template>
  <div class="teacher-courses">
    <!-- 统计卡片 -->
    <el-row :gutter="20" class="stats-row">
      <el-col :span="6">
        <el-card class="stat-card">
          <div class="stat-icon courses">📚</div>
          <div class="stat-info">
            <div class="stat-value">{{ statistics.totalCourses }}</div>
            <div class="stat-label">总课程数</div>
          </div>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card class="stat-card">
          <div class="stat-icon active">🎓</div>
          <div class="stat-info">
            <div class="stat-value">{{ statistics.activeCourses }}</div>
            <div class="stat-label">进行中</div>
          </div>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card class="stat-card">
          <div class="stat-icon students">👥</div>
          <div class="stat-info">
            <div class="stat-value">{{ statistics.totalStudents }}</div>
            <div class="stat-label">总学员数</div>
          </div>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card class="stat-card">
          <div class="stat-icon pending">⏰</div>
          <div class="stat-info">
            <div class="stat-value">{{ statistics.pendingCheckins }}</div>
            <div class="stat-label">待签到</div>
          </div>
        </el-card>
      </el-col>
    </el-row>

    <!-- 课程列表 -->
    <el-card class="courses-list">
      <template #header>
        <div class="card-header">
          <span>我的课程</span>
          <el-button v-permission="'courses:create'" type="primary" @click="handleCreate">
            创建课程
          </el-button>
        </div>
      </template>

      <el-table :data="courses" v-loading="loading">
        <el-table-column prop="title" label="课程名称" min-width="200" />
        <el-table-column label="报名人数" width="100" align="center">
          <template #default="{ row }">
            {{ row._count.enrollments }}
          </template>
        </el-table-column>
        <el-table-column label="状态" width="100">
          <template #default="{ row }">
            <el-tag :type="getStatusType(row.status)">
              {{ getStatusLabel(row.status) }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="开课时间" width="180">
          <template #default="{ row }">
            {{ formatDate(row.startTime) }}
          </template>
        </el-table-column>
        <el-table-column label="操作" width="400" fixed="right">
          <template #default="{ row }">
            <el-button link @click="handleViewStudents(row)">
              学员({{ row._count.enrollments }})
            </el-button>
            <el-button link type="primary" @click="handleStartCheckin(row)">
              开始签到
            </el-button>
            <el-button link @click="handleViewCheckins(row)">
              签到记录
            </el-button>
            <el-button link @click="handleViewEvaluations(row)">
              课程评价
            </el-button>
            <el-button link @click="handleManageMaterials(row)">
              课件管理
            </el-button>
            <el-button link type="primary" @click="handleEdit(row)">
              编辑
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
  </div>
</template>
```

### 2. 我的学员

```vue
<template>
  <div class="teacher-students">
    <el-card>
      <template #header>
        <div class="card-header">
          <span>我的学员 (共 {{ total }} 人)</span>
        </div>
      </template>

      <el-table :data="students" v-loading="loading">
        <el-table-column prop="realName" label="姓名" width="120" />
        <el-table-column prop="phone" label="手机号" width="130" />
        <el-table-column label="报名课程" min-width="200">
          <template #default="{ row }">
            <el-tag v-for="course in row.courses" :key="course.id" class="mr-5">
              {{ course.title }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="签到次数" width="100" align="center">
          <template #default="{ row }">
            {{ row.checkinCount }}
          </template>
        </el-table-column>
        <el-table-column label="评价次数" width="100" align="center">
          <template #default="{ row }">
            {{ row.evaluationCount }}
          </template>
        </el-table-column>
        <el-table-column label="操作" width="150">
          <template #default="{ row }">
            <el-button link type="primary" @click="handleViewDetail(row)">
              查看详情
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
  </div>
</template>
```

## 🎨 用户体验优化

### 1. 快捷操作
- 一键开启签到
- 快速查看学员
- 实时签到提醒

### 2. 数据可视化
- 签到率趋势图
- 评价分数分布
- 学员增长曲线

### 3. 移动端适配
- 手机扫码签到
- 移动端查看数据
- 推送签到提醒

## 📊 实施计划

### 第一阶段：基础功能（1周）
- ✅ 数据权限控制
- ✅ 我的课程页面
- ✅ 我的学员页面
- ✅ 基础签到功能

### 第二阶段：增强功能（1周）
- □ 签到统计和图表
- □ 评价统计和分析
- □ 课件管理优化
- □ 数据导出功能

### 第三阶段：体验优化（3天）
- □ 移动端适配
- □ 实时通知
- □ 性能优化
- □ 用户反馈收集

## 💡 技术要点

### 1. 数据权限过滤

```typescript
// 在 Service 层统一处理
async findCourses(userId: string, userRole: string, query: any) {
  const where: any = { ...query };
  
  // 教师只能看到自己的课程
  if (userRole === 'TEACHER') {
    where.teacherId = userId;
  }
  
  return this.prisma.course.findMany({ where });
}
```

### 2. 性能优化

```typescript
// 使用 include 优化查询
const courses = await this.prisma.course.findMany({
  include: {
    _count: {
      select: {
        enrollments: true,
        checkins: true
      }
    }
  }
});

// 批量查询减少数据库访问
const courseIds = courses.map(c => c.id);
const stats = await this.prisma.enrollment.groupBy({
  by: ['courseId'],
  where: { courseId: { in: courseIds } },
  _count: true
});
```

### 3. 缓存策略

```typescript
// 使用 Redis 缓存课程统计数据
const cacheKey = `teacher:${teacherId}:stats`;
let stats = await this.redis.get(cacheKey);

if (!stats) {
  stats = await this.calculateStats(teacherId);
  await this.redis.set(cacheKey, stats, 300); // 5分钟过期
}
```

## ✅ 验证清单

- [ ] 教师只能看到自己的课程
- [ ] 教师只能看到自己课程的学员
- [ ] 签到功能正常工作
- [ ] 评价数据正确显示
- [ ] 课件上传下载正常
- [ ] 数据统计准确
- [ ] 权限控制有效
- [ ] 页面响应流畅

这个方案是否符合您的期望？需要我开始实现哪个部分吗？





