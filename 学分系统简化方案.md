# 学分系统简化方案

## 🎯 需求说明

### 原始需求
- **只有一种学分**：不分企业学分和个人学分，统一为一个学分余额
- **学分属性区分**：
  - **可转赠学分**：管理员充值、课程奖励获得的学分，可以赠送给其他用户
  - **锁定学分**：企业管理员分配给企业用户的学分，不可转赠，只能用于购买课程

### 核心逻辑
1. **学分余额统一**：`balance` = 可转赠学分 + 锁定学分
2. **消费优先级**：报名课程时，优先使用锁定学分（企业分配的），不足时使用可转赠学分
3. **转赠限制**：只能转赠可转赠学分，锁定学分不可转赠

## 📊 数据库设计

### Credit 表结构（新设计）

```prisma
model Credit {
  id                  String   @id @default(uuid())
  userId              String   @unique
  balance             Int      @default(0)  // 当前学分余额（统一余额）
  total               Int      @default(0)  // 累计获得学分（统一总数）
  used                Int      @default(0)  // 累计消耗学分（统一消耗）
  
  // 学分来源统计（用于判断可转赠额度）
  transferableBalance Int      @default(0)  // 可转赠学分余额（个人学分）
  lockedBalance       Int      @default(0)  // 锁定学分余额（企业学分，不可转赠）
  
  // 关系：balance = transferableBalance + lockedBalance
  
  createdAt           DateTime @default(now())
  updatedAt           DateTime @updatedAt
  
  user                User     @relation(fields: [userId], references: [id])
  records             CreditRecord[]
}
```

### CreditRecordType 枚举（新增）

```prisma
enum CreditRecordType {
  EARN             // 获得学分（课程完成奖励）- 可转赠
  CONSUME          // 消耗学分（报名课程）
  REFUND           // 退回学分（退课）
  ADMIN_ADD        // 管理员增加 - 可转赠
  ADMIN_DEDUCT     // 管理员扣除
  CORPORATE_ALLOCATE  // 企业分配 - 锁定学分
  TRANSFER_OUT     // 转出学分（赠送给他人）
  TRANSFER_IN      // 转入学分（接收他人赠送）
}
```

### CreditSource 枚举（保持不变）

```prisma
enum CreditSource {
  PERSONAL    // 个人学分（可转赠）
  CORPORATE   // 企业学分（锁定）
}
```

## 🔄 迁移策略

### SQL 迁移脚本

```sql
-- 1. 添加新字段
ALTER TABLE `credits` 
ADD COLUMN `transferableBalance` INT NOT NULL DEFAULT 0 COMMENT '可转赠学分余额',
ADD COLUMN `lockedBalance` INT NOT NULL DEFAULT 0 COMMENT '锁定学分余额';

-- 2. 迁移现有数据
-- 将原来的 balance 设为可转赠学分
-- 将原来的 corporateBalance 设为锁定学分
UPDATE `credits` 
SET 
  `balance` = `balance` + COALESCE(`corporateBalance`, 0),
  `transferableBalance` = `balance`,
  `lockedBalance` = COALESCE(`corporateBalance`, 0),
  `total` = `total` + COALESCE(`corporateTotal`, 0)
WHERE TRUE;

-- 3. 验证数据（可选）
SELECT 
  userId,
  balance,
  transferableBalance,
  lockedBalance,
  (transferableBalance + lockedBalance) as calculated_balance
FROM `credits`
WHERE balance != (transferableBalance + lockedBalance);

-- 4. 删除旧字段（建议先备份，观察一段时间后再删除）
-- ALTER TABLE `credits` 
-- DROP COLUMN `corporateBalance`,
-- DROP COLUMN `corporateTotal`,
-- DROP COLUMN `corporateUsed`;
```

## 💻 后端实现

### 1. 学分消耗（优先使用锁定学分）

```typescript
async consumeCredit(userId: string, courseId: string, courseName: string, amount: number) {
  const credit = await this.prisma.credit.findUnique({ where: { userId } });
  
  if (credit.balance < amount) {
    throw new BadRequestException(`学分不足，当前余额: ${credit.balance}，需要: ${amount}`);
  }
  
  // 计算扣除方案：优先使用锁定学分
  const lockedUsed = Math.min(credit.lockedBalance, amount);
  const transferableUsed = amount - lockedUsed;
  
  await this.prisma.$transaction(async (tx) => {
    // 更新学分账户
    await tx.credit.update({
      where: { id: credit.id },
      data: {
        balance: credit.balance - amount,
        used: credit.used + amount,
        lockedBalance: credit.lockedBalance - lockedUsed,
        transferableBalance: credit.transferableBalance - transferableUsed,
      },
    });
    
    // 创建消耗记录
    await tx.creditRecord.create({
      data: {
        creditId: credit.id,
        type: 'CONSUME',
        amount: -amount,
        balance: credit.balance - amount,
        source: lockedUsed > 0 ? 'CORPORATE' : 'PERSONAL',
        courseId,
        courseName,
        remark: `报名课程《${courseName}》（锁定${lockedUsed}+可转赠${transferableUsed}）`,
      },
    });
  });
}
```

### 2. 增加学分（区分类型）

```typescript
async addCredit(
  userId: string, 
  amount: number, 
  remark: string,
  isLocked: boolean = false,  // 是否为锁定学分
  courseId?: string,
  courseName?: string
) {
  const credit = await this.prisma.credit.findUnique({ where: { userId } });
  
  await this.prisma.credit.update({
    where: { id: credit.id },
    data: {
      balance: credit.balance + amount,
      total: credit.total + amount,
      transferableBalance: isLocked ? credit.transferableBalance : credit.transferableBalance + amount,
      lockedBalance: isLocked ? credit.lockedBalance + amount : credit.lockedBalance,
    },
  });
  
  await this.prisma.creditRecord.create({
    data: {
      creditId: credit.id,
      type: isLocked ? 'CORPORATE_ALLOCATE' : (courseId ? 'EARN' : 'ADMIN_ADD'),
      amount,
      balance: credit.balance + amount,
      source: isLocked ? 'CORPORATE' : 'PERSONAL',
      courseId,
      courseName,
      remark,
    },
  });
}
```

### 3. 企业管理员分配学分

```typescript
async allocateCredit(adminId: string, dto: AllocateCreditDto) {
  const { toUserId, amount } = dto;
  
  // 验证企业管理员身份
  const organization = await this.validateCorpAdmin(adminId);
  await this.validateEmployeeBelongsToOrg(organization.id, toUserId);
  
  const adminCredit = await this.prisma.credit.findUnique({ where: { userId: adminId } });
  
  // 只能分配管理员的可转赠学分
  if (adminCredit.transferableBalance < amount) {
    throw new BadRequestException(`可转赠学分不足，当前: ${adminCredit.transferableBalance}，需要: ${amount}`);
  }
  
  await this.prisma.$transaction(async (tx) => {
    // 扣除管理员的可转赠学分
    await tx.credit.update({
      where: { userId: adminId },
      data: {
        balance: adminCredit.balance - amount,
        used: adminCredit.used + amount,
        transferableBalance: adminCredit.transferableBalance - amount,
      },
    });
    
    // 增加员工的锁定学分（不可转赠）
    await tx.credit.update({
      where: { userId: toUserId },
      data: {
        balance: { increment: amount },
        total: { increment: amount },
        lockedBalance: { increment: amount },  // 作为锁定学分
      },
    });
    
    // 记录转出和转入
    await tx.creditRecord.createMany({
      data: [
        {
          creditId: adminCredit.id,
          type: 'TRANSFER_OUT',
          amount: -amount,
          balance: adminCredit.balance - amount,
          source: 'PERSONAL',
          fromUserId: adminId,
          toUserId,
          remark: `分配${amount}学分给员工（企业管理员）`,
        },
        {
          creditId: employeeCredit.id,
          type: 'CORPORATE_ALLOCATE',
          amount,
          balance: employeeCredit.balance + amount,
          source: 'CORPORATE',
          fromUserId: adminId,
          toUserId,
          remark: `收到企业管理员分配的${amount}学分（锁定，不可转赠）`,
        },
      ],
    });
  });
}
```

### 4. 转赠学分（新功能）

```typescript
async transferCredit(fromUserId: string, toUserId: string, amount: number) {
  const fromCredit = await this.prisma.credit.findUnique({ where: { userId: fromUserId } });
  
  // 只能转赠可转赠学分
  if (fromCredit.transferableBalance < amount) {
    throw new BadRequestException(`可转赠学分不足，当前: ${fromCredit.transferableBalance}，需要: ${amount}`);
  }
  
  await this.prisma.$transaction(async (tx) => {
    // 扣除转出方的可转赠学分
    await tx.credit.update({
      where: { userId: fromUserId },
      data: {
        balance: fromCredit.balance - amount,
        transferableBalance: fromCredit.transferableBalance - amount,
      },
    });
    
    // 增加转入方的可转赠学分
    await tx.credit.update({
      where: { userId: toUserId },
      data: {
        balance: { increment: amount },
        total: { increment: amount },
        transferableBalance: { increment: amount },  // 转赠的学分也是可转赠的
      },
    });
    
    // 记录转账
    // ... 创建转出和转入记录
  });
}
```

## 🎨 前端展示

### 个人中心学分显示

```vue
<view class="credit-card">
  <!-- 总学分 -->
  <view class="credit-item">
    <Icon name="star" color="#FFD700" />
    <text class="label">总学分</text>
    <text class="value">{{ credit.balance }}</text>
  </view>
  
  <!-- 可转赠学分 -->
  <view class="credit-item">
    <Icon name="user" color="#52C41A" />
    <text class="label">可转赠</text>
    <text class="value">{{ credit.transferableBalance }}</text>
  </view>
  
  <!-- 锁定学分 -->
  <view class="credit-item">
    <Icon name="work" color="#FF6B6B" />
    <text class="label">锁定学分</text>
    <text class="value">{{ credit.lockedBalance }}</text>
    <text class="tip">仅可购课</text>
  </view>
</view>
```

### 报名时的学分提示

```
报名《课程名称》需要 5 学分

您的学分余额：10
- 锁定学分：3（优先使用）
- 可转赠学分：7

将扣除：
- 锁定学分：3
- 可转赠学分：2
```

## 📝 API 接口变化

### 获取学分信息

**请求**：`GET /api/credits/my`

**响应**：
```json
{
  "id": "credit-id",
  "userId": "user-id",
  "balance": 444,              // 总余额
  "total": 500,                // 累计获得
  "used": 56,                  // 累计消耗
  "transferableBalance": 100,  // 可转赠余额
  "lockedBalance": 344,        // 锁定余额（不可转赠）
  "records": [...]
}
```

### 转赠学分（新增）

**请求**：`POST /api/credits/transfer`

```json
{
  "toUserId": "user-id",
  "amount": 10
}
```

**响应**：
```json
{
  "success": true,
  "message": "转赠成功",
  "data": {
    "transferredAmount": 10,
    "remainingTransferable": 90
  }
}
```

## ✅ 优势

1. **统一余额**：用户只看到一个学分数字，简单直观
2. **灵活控制**：通过 `transferableBalance` 和 `lockedBalance` 灵活控制学分属性
3. **兼容性好**：保留原有字段，可平滑迁移
4. **审计完整**：通过 `CreditRecord` 记录每笔学分的来源和用途
5. **扩展性强**：未来可以轻松添加其他学分类型（如积分、券等）

## 🎯 下一步

1. ✅ 更新 Prisma Schema
2. ✅ 创建数据库迁移脚本
3. ⏳ 更新后端服务代码
4. ⏳ 更新前端显示
5. ⏳ 测试验证
6. ⏳ 部署上线

由于现在直接运行迁移有权限问题，建议：
1. 手动执行 SQL 迁移脚本
2. 然后重新生成 Prisma Client
3. 更新后端代码以适配新结构









